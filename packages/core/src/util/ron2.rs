...
fn extensions() -> Extensions {
	...
	exts |= Extensions::IMPLICIT_SOME;
	exts |= Extensions::UNWRAP_NEWTYPES;
	exts |= Extensions::UNWRAP_VARIANT_NEWTYPES;
	exts
}
...
pub fn from_str<'h, T>(s: &'h str) -> Result<T, ron_err::Ron>
where
	T: Deserialize<'h>
{
	ron::Options::default()
...
pub fn to_string_minified<T>(value: &T)
	-> Result<String, ron_err::Ron>
where
	T: ?Sized + Serialize
{
	ron::Options::default()
...
#[inline]
pub fn to_string_pretty<T>(value: &T)
	-> Result<String, ron_err::Ron>
where
	T: ?Sized + Serialize
{
...
#[inline]
pub fn to_string_pretty_extend<T, F>(value: &T, f: F)
	-> Result<String, ron_err::Ron>
where
	T: ?Sized + Serialize,
...
pub fn to_string_pretty_custom<T>(value: &T, config: PrettyConfig)
	-> Result<String, ron_err::Ron>
where
	T: ?Sized + Serialize
{
