...
(async () => {
	...
	const lines = fs.readFileSync(path.resolve(dest), "utf8")
		.split("\n");
	...
	const no_start = start_i < 0;
	const no_end = end_i < 0;

	if (no_start && no_end) {
		run_if_CI(
			() => console.log(`::error file=${dest}::start/end markers not found, refusing to continue`),
			() => console.log(`start/end markers not found in ${dest}, refusing to continue`)
		);
	} else if (no_start) {
		run_if_CI(
			() => console.log(`::error file=${dest}::start marker not found, refusing to continue`),
			() => console.log(`start marker not found in ${dest}, refusing to continue`)
		)
	} else if (no_end) {
		run_if_CI(
			() => console.log(`::error file=${dest}::end marker not found, refusing to continue`),
			() => console.log(`end marker not found in ${dest}, refusing to continue`)
		)
	}
